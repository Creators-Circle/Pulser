...
class SummaryMainPane extends Component {
  ...
  render () {
    if (this.props.summary.users) {
      ...
      const clickedUser = this.props.userId || '';
      ...
      const userClicks = this.props.summary.clicks.filter(click => click.user_id === clickedUser);
      const totalClicks = !clickedUser ? this.props.summary.clicks.length : userClicks.length;
      ...
      const questions = !clickedUser ? this.props.summary.questions
      ...
      const clickPerTime = {};
      const clicks = !this.props.userId ? this.props.summary.clicks : userClicks;
      ...
      const userUpvotes = this.props.summary.upvotes.filter(vote => vote.user_id === clickedUser)
        .map(upvote => upvote.question);

      const noOfUpvotes = userUpvotes.length;

      const thumbs = this.props.summary.thumbs.filter(thumb => thumb.user_id === clickedUser);
      const thumbsCount = {
        1: 0,
        2: 0,
        3: 0
      ...
      const topicsSummary = {};
      const topics = this.props.summary.thumbs;
